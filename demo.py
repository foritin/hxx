...
def calculate_gini_mean_diff(close, lengs):
    ...
    # 计算均线间的绝对差值
    abs_diff_1_4 = np.abs(sma1 - sma4)
    abs_diff_1_6 = np.abs(sma1 - sma6)
    abs_diff_1_9 = np.abs(sma1 - sma9)
    abs_diff_4_6 = np.abs(sma4 - sma6)
    abs_diff_4_9 = np.abs(sma4 - sma9)
    abs_diff_6_9 = np.abs(sma6 - sma9)
...
def calculate_tvi_heikin_ashi(TVI, current_idx):
    ...
    """
    # TVI蜡烛图计算
    cl = TVI.iloc[current_idx] if current_idx < len(TVI) else TVI.iloc[-1]
    op = TVI.iloc[current_idx - 1] if current_idx > 0 else cl
...
def calculate_sar(high, low, current_idx, AfStep, AfLimit=0.2):
    ...
    if current_idx == 0 or state.HHValue_prev is None or state.LLValue_prev is None or state.SAR_prev is None:
        ...
        state.HHValue_prev = high.iloc[current_idx]
        state.LLValue_prev = low.iloc[current_idx]
        state.SAR_prev = state.LLValue_prev
        state.ParOpen_prev = state.SAR_prev + state.Af_prev * (state.HHValue_prev - state.SAR_prev)

        if state.ParOpen_prev > low.iloc[current_idx]:
            state.ParOpen_prev = low.iloc[current_idx]
    ...
    # 更新最高值和最低值
    HHValue = max(high.iloc[current_idx], state.HHValue_prev)
    LLValue = min(low.iloc[current_idx], state.LLValue_prev)
    ...
    if state.TbPosition_prev == 1:
        if low.iloc[current_idx] <= state.ParOpen_prev:
            # 趋势反转：转为空头
            TbPosition = -1
            Transition = -1
            SAR = state.HHValue_prev
            HHValue = high.iloc[current_idx]
            LLValue = low.iloc[current_idx]
            Af = AfStep
            ParOpen = SAR + Af * (LLValue - SAR)

            if ParOpen < high.iloc[current_idx]:
                ParOpen = high.iloc[current_idx]
            if current_idx > 0 and ParOpen < high.iloc[current_idx - 1]:
                ParOpen = high.iloc[current_idx - 1]
        else:
            # 保持多头持仓
            TbPosition = 1
            SAR = state.ParOpen_prev

            if high.iloc[current_idx] > state.HHValue_prev and state.Af_prev < AfLimit:
            ...
            if ParOpen > low.iloc[current_idx]:
                ParOpen = low.iloc[current_idx]
            if current_idx > 0 and ParOpen > low.iloc[current_idx - 1]:
                ParOpen = low.iloc[current_idx - 1]

    # 空头持仓处理
    else:
        if high.iloc[current_idx] >= state.ParOpen_prev:
            # 趋势反转：转为多头
            TbPosition = 1
            Transition = 1
            ...
            if low.iloc[current_idx] < state.LLValue_prev and state.Af_prev < AfLimit:
                Af = min(state.Af_prev + AfStep, AfLimit)
            else:
                Af = state.Af_prev
...
def vip24_strategy(api: StrategyAPI):
    ...
    if state.ha_op_prev is None or state.ha_cl_prev is None:
        # 初始化HA值
        cl = TVI.iloc[current_idx] if not pd.isna(TVI.iloc[current_idx]) else 0
        op = TVI.iloc[current_idx - 1] if current_idx > 0 and not pd.isna(TVI.iloc[current_idx - 1]) else cl
    ...
    # 7. 计算最高价/最低价区间
    HH = high.rolling(X).max().iloc[current_idx]
    LL = low.rolling(X).min().iloc[current_idx]
    HH_prev = high.rolling(X).max().iloc[current_idx - 1] if current_idx > 0 else HH
    LL_prev = low.rolling(X).min().iloc[current_idx - 1] if current_idx > 0 else LL

    # 获取当前持仓
    current_pos = api.get_pos()

    # 开多条件：前一Bar的SAR转多 + 前一Bar的TVI趋势变化 + 当前Bar突破前高
    if prev_transition == 1 and prev_trend_change and high.iloc[current_idx] >= HH_prev and current_pos == 0:

        entry_price = max(open_p.iloc[current_idx], HH_prev)
        api.buy(volume=Lots, order_type="next_bar_open")
        api.log(f"开多: {Lots}手 @ {entry_price:.2f} (前Bar SAR转多+TVI变化+突破前高)")

        # 初始化跟踪止损
        state.HighestLowAfterEntry = open_p.iloc[current_idx]
        state.entPrice = open_p.iloc[current_idx]
        state.entBar = current_idx

    # 开空条件：前一Bar的SAR转空 + 前一Bar的TVI趋势变化 + 当前Bar突破前低
    elif prev_transition == -1 and prev_trend_change and low.iloc[current_idx] <= LL_prev and current_pos == 0:

        entry_price = min(open_p.iloc[current_idx], LL_prev)
        api.sellshort(volume=Lots, order_type="next_bar_open")
        api.log(f"开空: {Lots}手 @ {entry_price:.2f} (前Bar SAR转空+TVI变化+突破前低)")

        # 初始化跟踪止损
        state.LowestHighAfterEntry = open_p.iloc[current_idx]
        state.entPrice = open_p.iloc[current_idx]
        state.entBar = current_idx
    ...
    if current_pos > 0 and state.entBar is not None and current_idx > state.entBar:
        ...
        # 计算当前Bar的止损线
        Trailing_Stop_L = state.HighestLowAfterEntry - open_p.iloc[current_idx] * TrailingStopRate / 1000

        # 关键修正：如果前一个Bar有止损线，使用前一个Bar的止损线进行比较
        if hasattr(state, "Trailing_Stop_L_prev") and current_idx > state.entBar:
            if low.iloc[current_idx] <= state.Trailing_Stop_L_prev:
                exit_price = min(open_p.iloc[current_idx], state.Trailing_Stop_L_prev)
        ...
        # 计算当前Bar的止损线
        Trailing_Stop_S = state.LowestHighAfterEntry + open_p.iloc[current_idx] * TrailingStopRate / 1000

        # 关键修正：如果前一个Bar有止损线，使用前一个Bar的止损线进行比较
        if hasattr(state, "Trailing_Stop_S_prev") and current_idx > state.entBar:
            if high.iloc[current_idx] >= state.Trailing_Stop_S_prev:
                exit_price = max(open_p.iloc[current_idx], state.Trailing_Stop_S_prev)
    ...
    if current_idx % 100 == 0:
        ...
        api.log(f"HH_prev={HH_prev:.2f}, LL_prev={LL_prev:.2f}, High={high.iloc[current_idx]:.2f}, Low={low.iloc[current_idx]:.2f}")

    # 10. 记录所有交易信号（无论是否开仓）
    if prev_transition == 1 and prev_trend_change and high.iloc[current_idx] >= HH_prev:
        if current_pos == 0:
            # 已经在上面记录了开仓日志
            pass
        else:
    ...
    elif prev_transition == -1 and prev_trend_change and low.iloc[current_idx] <= LL_prev:
        if current_pos == 0:
            # 已经在上面记录了开仓日志
            pass
        else:
...
if __name__ == "__main__":
    ...
    # 设置基础配置
    backtester.set_base_config({"username": API_USERNAME, "password": API_PASSWORD, "use_cache": True, "save_data": True, "debug": True})
    ...
    backtester.add_symbol_config(
        ...
        config={
            ...
            "periods": [
                {"kline_period": "30m", "adjust_type": "1"},
            ],
        },
    )
    ...
    strategy_params = {
        "lengs": 75,  # ATR周期参数
        "AfStep": 0.02,  # SAR加速因子（修正为与TBQuant一致）
        "X": 25,  # 区间周期
        "Lots": 1,  # 交易手数（固定）
        "TrailingStopRate": 20,  # 跟踪止损比例（修正为与TBQuant一致）
        "AfLimit": 0.2,  # SAR加速因子上限
    }

    # 运行回测
    results = backtester.run(strategy=vip24_strategy, initialize=initialize, strategy_params=strategy_params)
